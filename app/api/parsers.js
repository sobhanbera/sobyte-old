            ...
            case 'Song':
                result.content.push(
                    Object.freeze({
                        type: _.lowerCase(
                            _.nth(
                                utils.fv(_.nth(flexColumn, 1), 'runs:text'),
                                0,
                            ),
                        ),
                        videoId: utils.fv(
                        ...
                        artist: (function () {
                            var a = [],
                                c = _.nth(
                                    utils.fv(_.nth(flexColumn, 1), 'runs'),
                                    2,
                                )
                            if (Array.isArray(c)) {
            ...
            case 'Artist':
                result.content.push(
                    Object.freeze({
                        ...
                        browseId: utils.fv(
                            _.at(sectionContext, 'navigationEndpoint'),
                            'browseEndpoint:browseId',
                        ),
                        name: utils.fv(_.nth(flexColumn, 0), 'runs:text'),
                        thumbnails: utils.fv(
...
exports.parseSongSearchResult = context => {
    ...
    var sectionList = utils.fv(context, 'musicResponsiveListItemRenderer', true)
    ...
    sectionList.forEach(sectionContext => {
        ...
        result.content.push({
            ...
            duration: utils.hms2ms(
                _.last(utils.fv(flexColumn[1], 'runs:text', true)),
            ),
            thumbnails: utils.fv(
                sectionContext,
                'musicThumbnailRenderer:thumbnails',
...
exports.parseAlbumSearchResult = context => {
    ...
    sectionList.forEach(sectionContext => {
        ...
        result.content.push(
            Object.freeze({
                type: _.lowerCase(
                    _.first(utils.fv(_.nth(flexColumn, 1), 'runs:text')),
                ),
                browseId: utils.fv(
                    _.at(sectionContext, 'navigationEndpoint'),
                    'browseEndpoint:browseId',
                ...
                artist: _.join(
                    _.filter(
                        utils
                            .fv(_.nth(flexColumn, 1), 'runs:text')
                            .slice(1, -1),
                        v => ' • ' != v && true,
                    ),
                    '',
...
exports.parseArtistPage = context => {
    ...
    const headerContext = utils.fv(context, 'musicImmersiveHeaderRenderer')
    result.name = utils.fv(_.at(headerContext, 'title'), 'text')
    result.thumbnails = utils.fv(
        _.at(headerContext, 'thumbnail'),
        'musicThumbnailRenderer:thumbnails',
    )
    ...
    if (!Array.isArray(descriptionContext)) {
        result.description = utils.fv(
            _.at(descriptionContext, 'description'),
            'text',
        )
        result.views = _.parseInt(
            _.replace(
                _.nth(
                    _.split(
                        utils.fv(_.at(descriptionContext, 'subheader'), 'text'),
                        ' ',
                    ),
                    0,
                ),
    ...
    const nextMusicNavigation = utils.fv(
        context,
    ...
    utils
        .fv(context, 'musicShelfRenderer:musicResponsiveListItemRenderer')
        .forEach(itemContext => {
            const flexColumn = _.concat(
                utils.fv(
                    itemContext,
    ...
    utils.fv(context, 'musicCarouselShelfRenderer').forEach(carouselContext => {
        const carouselName = _.lowerCase(
            utils.fv(
                carouselContext,
        ...
        if (['albums', 'singles', 'videos'].includes(carouselName)) {
            const nextCarouselNavigation = utils.fv(
                carouselContext,
                    ...
                    switch (carouselName) {
                        case 'singles':
                            result.products[carouselName].content.push({
                                type: 'single',
                                browseId: utils.fv(
                                    _.at(itemContext[i], 'navigationEndpoint'),
                                    'browseEndpoint:browseId',
                                ),
                                name: utils.fv(
                ...
                switch (carouselName) {
                    case 'singles':
                        result.products[carouselName].content.push({
                            type: 'single',
                            browseId: utils.fv(
                                _.at(itemContext, 'navigationEndpoint'),
                                'browseEndpoint:browseId',
                            ),
                            name: utils.fv(_.at(itemContext, 'title'), 'text'),
                            year: utils.fv(
                                _.at(itemContext, 'subtitle'),
                                'text',
                            ),
                            thumbnails: utils.fv(
                                itemContext,
                    ...
                    case 'albums':
                        result.products[carouselName].content.push({
                            type: _.nth(
                                utils.fv(_.at(itemContext, 'subtitle'), 'text'),
                                0,
                            ),
                            browseId: utils.fv(
                                _.at(itemContext, 'navigationEndpoint'),
...
exports.parsePlaylistPage = context => {
    ...
    if (!_.has(context, 'continuationContents')) {
        const pageHeader = utils.fv(context, 'musicDetailHeaderRenderer')
        result.title = utils.fv(_.at(pageHeader, 'title'), 'runs:text')
        result.owner = _.nth(
            utils.fv(_.at(pageHeader, 'subtitle'), 'runs:text'),
            2,
        )
        result.trackCount = parseInt(
            _.words(
                _.nth(
                    utils.fv(_.at(pageHeader, 'secondSubtitle'), 'runs:text'),
                    0,
                ),
            ),
        )
        result.dateYear = _.nth(
            utils.fv(_.at(pageHeader, 'subtitle'), 'runs:text'),
            4,
        )
        result.thumbnails = utils.fv(
    ...
    const itemContext = utils.fv(context, 'musicResponsiveListItemRenderer')
    if (Array.isArray(itemContext)) {
        for (let i = 0; i < itemContext.length; i++) {
            const flexColumn = utils.fv(
                itemContext[i],
...
exports.parseAlbumPage = context => {
    ...
    const albumRelease = utils.fv(context, 'musicAlbumRelease')
    ...
    const albumReleaseDetail = utils.fv(context, 'musicAlbumReleaseDetail')
    result.description = albumReleaseDetail.description

    const albumArtist = utils.fv(context, 'musicArtist')
    ...
    const albumTrack = utils.fv(context, 'musicTrack')
...
exports.parseNextPanel = context => {
    ...
    utils
        .fv(panelContext, 'playlistPanelVideoRenderer')
        .forEach(itemContext => {
            result.content.push({
                index: _.nth(
                    _.at(itemContext, 'navigationEndpoint.watchEndpoint.index'),
                    0,
                ),
